...
export default function LearnerView({ onSubmitRequest }) {
 ...
 // Request form state.
  ...
  const [loading, setLoading] = useState(false); // Tracks whether the AI request is in progress (used for "Thinking..." + disabling buttons)
  const [error, setError] = useState(""); // Stores a user-friendly error message if the AI request fails

  const handleAskAssistant = async () => {
    // Runs when the learner clicks "Ask" to call the backend AI endpoint
    try {
      setError(""); // Clear any previous error so the UI does not show old errors on a new request
      setLoading(true); // Turn on loading state so the UI can show "Thinking..." and prevent double submits

      const data = await sendAiMessage({
        // Calls our frontend helper which POSTs to /api/ai/chat
        topic, // Topic selected in the learner request (used by the backend to guide the AI response)
        message: question, // The learner's typed question, sent as "message" because that is what the backend expects
      });

      setAssistantRes(data); // Save the AI response so the UI can render reply + practice question
    } catch (err) {
      console.error(err); // Log the real error for debugging (network issue, backend error, invalid key, etc.)
      setError("AI request failed. Please try again."); // Show a safe message in the UI (do not expose technical details)
      setAssistantRes(null); // Clear old AI output so we do not display stale results after a failure
    } finally {
      setLoading(false); // Always turn off loading, whether the request succeeded or failed
    }
  };
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 16 }}>
            ...
            <div style={card}>
              ...
              <p style={{ marginTop: -6, color: colors.muted }}>
                Quick hints and practice while you wait. Not a replacement for a
                tutor.
              </p>
              ...
              {!submitted ? (
                ...
                <>
                  ...
                  {loading && (
                    <p style={{ marginTop: 10, color: colors.muted }}>
                      Thinking...
                    </p>
                  )}
                  {error && (
